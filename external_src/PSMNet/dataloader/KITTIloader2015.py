...
def dataloader(filepath):
  ...
  left = sorted(glob.glob(os.path.join(filepath, left_fold, '*.png')))
  right = sorted(glob.glob(os.path.join(filepath, right_fold, '*.png')))
  disp = sorted(glob.glob(os.path.join(filepath, disp_L, '*.png')))

  left_train = left[:320]
  right_train = right[:320]
  disp_train_L = disp[:320]
  left_val = left[320:]
  right_val = right[320:]
  disp_val_L = disp[320:]
