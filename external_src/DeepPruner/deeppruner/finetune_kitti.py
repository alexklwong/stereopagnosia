...
TestImgLoader = torch.utils.data.DataLoader(
         ...
         #batch_size=8, shuffle=False, num_workers=2, drop_last=False)
...
def add_input_diversity(image0, image1, disp_gt):

    p = 0.5        # Transformation Probability

    chance = torch.rand(1)
    if chance >= p:
        #print("not transforming")
        return image0, image1, disp_gt

    assert(image0.shape == image1.shape)
    #assert(image0.shape == disp_gt.shape)

    disp_gt = disp_gt.unsqueeze(1)

    #print("transforming")
    n, c, h, w = image0.shape

    new_h = random.randint(int(h-h/10), h)
    new_w = random.randint(int(w-w/10), w)

    top_pad = random.randint(0, h-new_h)
    bottom_pad = h - new_h - top_pad

    left_pad = random.randint(0, w-new_w)
    right_pad = w - new_w - left_pad

    resized0 = F.interpolate(image0, (new_h, new_w), mode='bilinear')
    transformed0 = F.pad(resized0, (left_pad, right_pad, top_pad, bottom_pad))

    resized1 = F.interpolate(image1, (new_h, new_w), mode='bilinear')
    transformed1 = F.pad(resized1, (left_pad, right_pad, top_pad, bottom_pad))

    resized_gt = F.interpolate(disp_gt, (new_h, new_w), mode='nearest')
    transformed_gt = F.pad(resized_gt, (left_pad, right_pad, top_pad, bottom_pad))

    transformed_gt *= (new_w / w)

    assert(image0.shape == transformed0.shape)
    assert(image1.shape == transformed1.shape)
    assert(disp_gt.shape == transformed_gt.shape)

    transformed_gt = transformed_gt.squeeze(1)

    return transformed0, transformed1, transformed_gt
...
def train(imgL, imgR, disp_L, iteration, epoch):
    ...
    imgL, imgR, disp_L = add_input_diversity(imgL, imgR, disp_L)
